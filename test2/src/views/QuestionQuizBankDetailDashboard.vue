                    ...
                    <div v-else>
                        <table class="table table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr v-for="(question_quiz_bank, index) in list_question_quizs_local"
                                    ...
                                    <td><span class="badge bg-label-success"> Đáp án {{
                                        parseInt(question_quiz_bank.right_answer) + 1 }}</span></td>
                                    <td>

                                        <ul v-if="question_quiz_bank.teacher"
                                            class="list-unstyled users-list m-0 avatar-group d-flex align-items-center">
                                            <li data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="top"
                                                class="avatar avatar-xs pull-up" title=""
                                                data-bs-original-title="Lilian Fuller">
                                                <img :src="`http://localhost:3000/files/` + question_quiz_bank.teacher.avatar"
                                                    alt="Avatar" class="rounded-circle">
                                            </li>
                                            {{ question_quiz_bank.teacher.teacher_id }}
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end" aria-labelledby="cardOpt4">
                                            <router-link class="dropdown-item" :to="{
                                                ...
                                                params: { id_question: question_quiz_bank._id, id_subject: route.params.id_subject, id_question_bank: route.params.id_question_bank }

                                            }"> Chỉnh sửa</router-link>
                                            <span @click="delete_question(question_quiz_bank._id)"
                                                class="dropdown-item"> Xóa khỏi ngân hàng này</span>

                                            <span class="dropdown-item" @click="duplicate(question_quiz_bank._id)">
                                                Nhân đôi</span>
...
// <a class="dropdown-item" href="javascript:void(0);">Delete</a> -->
            ...
            <!-- <div class="toast-container position-fixed bottom-0 end-0 me-4" v-show="state_question_quiz">
                <div id="liveToast" class="bs-toast toast fade show bg-primary" role="alert" aria-live="assertive"
                    aria-atomic="true">
                    <div class="toast-header">
                        <i class="bx bx-bell me-2"></i>
...
import QuestionQuizBankDetailDashboardTable from "@/components/QuestionQuizBankDetailDashboardTable.vue";
...
const { action_one_question_quiz_bank,action_update_question_quiz_bank, action_add_id_question_quiz_bank,action_all_question_quiz_banks, reset_state_question_quiz_bank, action_delete_question_quiz_bank } = useQuestion_quiz_bankStore();
import { useQuestion_quizStore } from "../stores/question_quiz.js"
const { get_all_question_quizs, get_lenght_question_quiz, get_one_question_quiz, state_question_quiz } = storeToRefs(useQuestion_quizStore());
const { action_all_question_quizs_by_teacher,action_add_question_quiz,action_one_question_quiz, action_list_question_quiz_by_question_bank } = useQuestion_quizStore();
...
async function duplicate(id_question) {
    await action_one_question_quiz(id_question);
    const one_question = get_one_question_quiz.value;
    await action_add_question_quiz(one_question);
...
async function delete_question(id_question) {
  ...
  let list_question_update = get_one_question_quiz_bank.value.list_question.filter((id) => {
      return id_question != id;
  })
 
   await action_update_question_quiz_bank(route.params.id_question_bank, {list_question: list_question_update});
...
const list_question_quizs_local = ref([]);
async function get_list_question_submit_code_banks() {
    list_question_quizs_local.value = get_all_question_quizs.value
    if (list_question_quizs_local.value) {

        for (const [index, question] of list_question_quizs_local.value.entries()) {

            if (question.teacher_id) {
                await action_one_teacher(question.teacher_id);
                let teacher = get_one_teacher.value;
                if (teacher._id) {
                    list_question_quizs_local.value[index].teacher = teacher;
                }
            }
