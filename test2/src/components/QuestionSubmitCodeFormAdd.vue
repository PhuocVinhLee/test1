...
async function submit() {
    ...
    if (!file.value.err_size && !file.value.err_type) {
        ...
        if (check_answer && !file.value.err_size && !file.value.err_type) {
            console.log(question_submit_code.answer)
            console.log(question_submit_code.question_submit_code_describe)
            ...
            //   data.append('answer', JSON.stringify(list_input_local.value))
            // const blob = new Blob([list_input_local.value], {type: "application/json"});
            // data.append('answer', blob)
            //  data.append("answer[][1]", list_input_local.value[0])


            await action_add_question_submit_code(data);
                            ...
                            <Form @submit="submit()" class="row g-3  " id="exam_add" :validation-schema="productFormSchema"
                               ...
                               <span v-html="question_submit_code.question_submit_code_describe"></span>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="mb-2" v-show="image"><img :src="image" id="image" alt="user-avatar"
